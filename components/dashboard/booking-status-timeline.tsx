...
export function BookingStatusTimeline({ logs }: { logs: StatusLogItem[] }) {
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-4">
      <div className="mb-3">
        <h3 className="text-sm font-semibold text-gray-900">Lịch sử trạng thái</h3>
        <p className="text-xs text-gray-500">Timeline thay đổi booking</p>
      </div>

      {logs.length === 0 ? (
        <p className="text-sm text-gray-500">Chưa có log trạng thái.</p>
      ) : (
          ...
          {logs.map((log, index) => (
            <div key={log.id} className="flex gap-3">
              <div className="flex w-5 flex-col items-center pt-1">
                ...
                {index < logs.length - 1 ? <span className="mt-1 h-full w-px bg-gray-200" /> : null}
              </div>

              <div className="flex-1 rounded-xl bg-gray-50 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-sm font-medium text-gray-900">
                    {log.old_status ? `${getBookingStatusLabel(log.old_status)} → ` : ""}
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Changed by: {log.changed_by_role || "system"}
                </p>
