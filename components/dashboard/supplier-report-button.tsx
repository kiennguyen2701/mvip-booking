...
export default function SupplierReportButton() {
  ...
  // Cần mounted để dùng createPortal (tránh SSR mismatch)
  useEffect(() => {
    setMounted(true);
    const now = new Date();
  ...
  const modal = (
    ...
    >
      <div
        ref={modalRef}
        className="w-full max-w-sm rounded-3xl border border-white/10 bg-[#11100c] shadow-2xl shadow-black/60"
  ...
  return (
    <>
      ...
      {/* Portal: render ra document.body để thoát khỏi stacking context của backdrop-blur cha */}
