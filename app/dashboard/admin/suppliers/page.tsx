...
export default async function AdminSuppliersPage() {
  ...
  return (
    <main className="min-h-screen bg-[#fbf7ef]">
      <div className="fixed right-4 top-1/2 z-[9990] hidden -translate-y-1/2 flex-col gap-2 md:flex">
        <a href="#create-supplier" className="rounded-2xl bg-slate-950 px-4 py-3 text-center text-xs font-black text-white shadow-xl shadow-black/20 transition hover:bg-slate-800">
          Create
        </a>
        <a href="#list-restaurants" className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-center text-xs font-black text-slate-800 shadow-xl shadow-black/10 transition hover:bg-slate-50">
          Restaurants
        </a>
        <a href="#list-suppliers" className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-center text-xs font-black text-slate-800 shadow-xl shadow-black/10 transition hover:bg-slate-50">
          Suppliers
        </a>
        <Link href="/dashboard/admin/supplier-requests" className="relative rounded-2xl bg-amber-300 px-4 py-3 text-center text-xs font-black text-slate-950 shadow-xl shadow-amber-900/20 transition hover:bg-amber-200">
          Requests
          {pendingRequests > 0 && (
            <span className="absolute -right-2 -top-2 flex h-5 min-w-5 items-center justify-center rounded-full bg-red-600 px-1.5 text-[10px] font-black text-white">
              {pendingRequests}
            </span>
          )}
        </Link>
      ...
      <div className="fixed bottom-4 left-3 right-3 z-[9990] grid grid-cols-4 gap-2 md:hidden">
        <a href="#create-supplier" className="rounded-2xl bg-slate-950 px-2 py-3 text-center text-[11px] font-black text-white shadow-xl">Create</a>
        <a href="#list-restaurants" className="rounded-2xl border border-slate-200 bg-white px-2 py-3 text-center text-[11px] font-black text-slate-800 shadow-xl">Res</a>
        <a href="#list-suppliers" className="rounded-2xl border border-slate-200 bg-white px-2 py-3 text-center text-[11px] font-black text-slate-800 shadow-xl">Sup</a>
        <Link href="/dashboard/admin/supplier-requests" className="relative rounded-2xl bg-amber-300 px-2 py-3 text-center text-[11px] font-black text-slate-950 shadow-xl">
          Req
          {pendingRequests > 0 && (
      ...
      <div className="mx-auto max-w-7xl px-4 pb-6 pt-8 md:px-6">
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 md:px-6">
        <section id="create-supplier" className="scroll-mt-[90px] rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-black text-slate-950">Create Supplier</h2>
        ...
        <section id="list-restaurants" className="scroll-mt-[90px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-3 border-b border-slate-100 p-5 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="text-lg font-black text-slate-950">List Restaurants</h2>
              <p className="mt-1 text-sm text-slate-500">
                ...
                {restaurantRows.map((restaurant) => (
                  <tr key={restaurant.id} className="hover:bg-slate-50">
                    <td className="px-5 py-4">
                      <p className="font-bold text-slate-950">{restaurant.name || 'Restaurant'}</p>
                      <p className="mt-1 text-xs text-slate-400">{restaurant.slug || '-'}</p>
                    </td>

                    <td className="px-5 py-4 text-slate-600">
                      {restaurant.supplier_id ? supplierNameMap.get(restaurant.supplier_id) || 'Supplier' : '-'}
                    </td>
                    ...
                    <td className="px-5 py-4 text-slate-600">{restaurant.phone || '-'}</td>
        ...
        <section id="list-suppliers" className="scroll-mt-[90px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 p-5">
            <h2 className="text-lg font-black text-slate-950">List Suppliers</h2>
            <p className="mt-1 text-sm text-slate-500">
            ...
            <table className="w-full min-w-[1050px] text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {supplierRows.map((supplier) => {
                  ...
                  const activeRestaurants = supplier.restaurants?.filter((item) => item.is_active).length || 0;

                  return (
                    <tr key={supplier.id} className="hover:bg-slate-50">
                      <td className="px-5 py-4">
                        <p className="font-bold text-slate-950">{getSupplierName(supplier)}</p>
                        <p className="mt-1 text-xs font-semibold text-emerald-600">active</p>
                      </td>

                      <td className="px-5 py-4 text-slate-600">{supplier.login_email || supplier.email || '-'}</td>
                      ...
                      <td className="px-5 py-4 text-slate-600">{totalRestaurants}</td>
                      <td className="px-5 py-4 text-slate-600">{activeRestaurants}</td>
